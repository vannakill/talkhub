...
const PostDetails = () => {
    ...
    const handleDelete = async ()=>{
      ...
      const getInviteLink = () => {
        return `localhost:5173/post/${postId}`
    }
    ...
    {post.map((data) => [ 
        <div className="post" key={data.id}>
            <div className="user-info">
                <p className="post-nickname">{data.username} &nbsp;  Опубликовано: {moment(data.createdat).fromNow()}</p>  
            </div>
            ...
            <div className='content'>
                ...
                />
                <p dangerouslySetInnerHTML={{ __html: data.desc}} className='post-desc'></p>
                <hr/>
                <div className="tags">
                { currentUser &&   (currentUser.username === data.username || currentUser.id === 1)  &&(
                    <button id='post' onClick={handleDelete}>Удалить</button>
                )}
